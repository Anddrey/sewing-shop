	...
	class ThemeControlHelper extends Controller{
		...
		private function autoLoadThemeCss(){
			...
			if( $this->skin ){
			 	...
			 	if( is_file($this->_themeDir . '/stylesheet/skins/'.$this->skin.'/font.css') ){
		 	 		$this->addCss( $this->themeURL.'skins/'.$this->skin.'/font.css' );
		 	 		$skinfont = true;
			 	}		
			} else {
			...
			if( $skinfont === false && is_file($this->_themeDir . '/stylesheet/font.css') ){
		 	 	$this->addCss( $this->themeURL.'font.css' );
		 	 	$skinfont = true;
			}		
